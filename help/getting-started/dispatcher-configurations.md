...
웹 서버 및 Dispatcher 구성 파일이 표준 AEM 컨텐츠 패키지와 함께 Git 저장소에 저장된 경우 Cloud Manager에서 배포할 수 있습니다.

이 기능을 사용하기 위해 Maven 빌드는 최소 두 개의 디렉터리(`conf` 및 `conf.d`)가 포함된 .zip 파일을 생성합니다. 이 .zip 파일을 `maven-assembly-plugin`을 사용하여 생성할 수 있습니다.

Cloud Manager가 기본 제공 [프로젝트 생성 마법사](/help/getting-started/using-the-wizard.md)를 사용하여 생성한 프로젝트는 자동으로 올바른 Maven 프로젝트 구조가 생성됩니다. Adobe Managed Services(AMS)를 처음 사용하는 경우에는 이 방법을 사용하는 것이 좋습니다.

Dispatcher 인스턴스에 배포하면 인스턴스의 디렉터리가 Git 저장소의 디렉터리로 바뀝니다. 웹 서버 및 Dispatcher 구성 파일에 종종 환경별 세부 정보가 필요하므로 `/etc/sysconfig/httpd`에서 적절한 환경 변수를 올바르게 설정하려면 고객 지원 팀과 공동 작업하십시오.

## 기존 관리 서비스 고객에 대한 Dispatcher 구성 {#steps-for-configuring-dispatcher}

초기 Dispatcher 구성을 완료하려면 다음을 수행하십시오.

1. 고객 지원 팀에서 현재 프로덕션 구성 파일을 가져옵니다.
1. 게시 렌더러 IP와 같은 환경 관련 데이터를 제거하고 변수로 바꿉니다.
1. 각 대상 Dispatcher의 키-값 쌍에 필요한 변수를 정의하고 각 인스턴스의 [variables](https://experienceleague.adobe.com/en/docs/experience-manager-learn/ams/dispatcher/variables) 폴더에 추가하십시오.
1. 스테이징 환경에서 업데이트된 구성을 테스트합니다.
1. 테스트가 완료되면 고객 지원 팀에 프로덕션에 배포하도록 요청합니다.
1. Git 저장소에 파일을 커밋합니다.
1. Cloud Manager를 통해 배포합니다.

>[!NOTE]
>
>Dispatcher 및 웹 서버 구성을 Git 저장소로 마이그레이션하는 작업은 Cloud Manager 온보딩 중에 수행되지만 나중에 수행할 수도 있습니다.

### 예 {#example}

특정 파일 및 디렉터리 구조는 프로젝트의 세부 사항에 따라 다르지만, 이 예에서는 Apache 및 Dispatcher 구성을 포함하도록 프로젝트를 구성하는 방법에 대한 명확한 설명을 제공합니다.

1. `dispatcher`라는 하위 디렉터리를 만듭니다.

   여기에서 임의의 이름을 사용하되, 이 단계에서 작성된 디렉터리 이름은 1단계에서 사용한 이름과 같아야 합니다.

1. 이 하위 디렉터리에는 `maven-assembly-plugin`을(를) 사용하여 Dispatcher .zip 파일을 작성하는 Maven 모듈이 포함되어 있습니다. `dispatcher` 디렉터리에서 이 내용이 포함된 `pom.xml` 파일을 만들고 필요에 따라 `parent` 참조 `artifactId` 및 `name`를 변경합니다.
   ...
   * 1단계에서와 같이 artifactId와 name은 다른 값이 될 수 있습니다. `dispatcher`이(가) 여기에 예로 사용되었습니다.

1. `maven-assembly-plugin`에서 .zip 파일을 만드는 방법을 정의하려면 `descriptor`이(가) 필요합니다. 이 설명자를 만들려면 `dispatcher` 하위 디렉터리에 다음 내용이 포함된 `assembly.xml`이라는 이름의 파일을 만듭니다. 이 파일 이름은 위의 `pom.xml` 파일의 26행에 있습니다.
...
1. 마지막으로 프로젝트의 루트에 있는 `pom.xml` 파일에 Dispatcher 모듈을 포함할 `<module>` 요소를 추가합니다.
...
1. 테스트하려면 프로젝트 루트 디렉터리에서 `mvn clean package`를 실행합니다. 출력에 이와 같은 선이 표시됩니다.
