...
>[!IMPORTANT]
>
>Beta 릴리스에는 결함이 포함되어 있으며 어떠한 종류의 보증도 없이 &quot;있는 그대로&quot; 제공됩니다. Adobe은 베타 릴리스를 유지, 수정, 업데이트, 변경, 수정 또는 기타 지원(Adobe 지원 서비스 또는 기타 방식으로)할 의무가 없습니다. 고객은 위험을 감수하고 Beta 릴리스를 사용하며 Beta 릴리스의 올바른 기능이나 성능 또는 함께 제공되는 설명서나 자료에 의존하지 않습니다. Beta의 기능 및 API는 예고 없이 변경될 수 있습니다. Beta 릴리스를 사용하는 것은 전적으로 고객 자신의 책임입니다.
...
새 빌드 모델은 모듈 수준 캐싱을 사용하여 전체 리포지토리가 아닌 변경된 모듈만 컴파일하므로 빌드 시간이 단축됩니다. 코드 품질 및 전체 스택 파이프라인에 적용됩니다.
