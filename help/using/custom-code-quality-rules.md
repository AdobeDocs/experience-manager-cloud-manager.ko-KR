...
>[!NOTE]
>
>여기에 제공된 코드 샘플은 설명 목적으로만 제공됩니다. 개념 및 품질 규칙에 대해 알아보려면 [SonarQube의 개념 설명서](https://docs.sonarsource.com/sonarqube-server)를 참조하세요.
...
### 외부에서 제어하는 형식 문자열 사용 안 함 {#do-not-use-format-strings-which-may-be-externally-controlled}
...
외부 소스의 형식 문자열(예: 요청 매개변수 또는 사용자 생성 콘텐츠)을 사용하면 애플리케이션이 서비스 거부 공격에 노출될 수 있습니다. 형식 문자열이 외부에서 제어되지만 신뢰할 수 있는 소스에서만 허용되는 경우도 있습니다.
...
피해야 할 또 다른 일반적인 패턴은 메시지를 기록한 다음 즉시 예외를 발생시키는 것입니다. 이 문제는 예외 메시지가 로그 파일에 중복됨을 나타냅니다.
...
가장 좋은 방법은 로그 메시지가 애플리케이션에서 예외가 발생한 위치에 대한 상황별 정보를 제공하는 것입니다. 스택 추적을 사용하여 컨텍스트를 결정하는 동안 일반적으로 로그 메시지는 읽고 이해하는 것이 더 쉽습니다. 따라서 예외를 기록할 때 예외 메시지를 로그 메시지로 사용하는 것은 잘못된 관행입니다. 예외 메시지에는 무엇이 잘못되었는지 자세히 설명되어 있습니다. 반대로 로그 메시지는 예외 발생 시 애플리케이션이 무엇을 하고 있었는지에 대해 판독기에 알립니다. 예외 메시지는 계속 기록됩니다. 고유한 메시지를 지정하면 로그를 쉽게 이해할 수 있습니다.
...
### catch 블록에 로그인하는 것은 WARN 또는 ERROR 수준입니다. {#logging-in-catch-blocks-should-be-at-the-warn-or-error-level}
...
로그 메시지를 이해할 때 컨텍스트는 중요합니다. `Exception.printStackTrace()`을(를) 사용하면 스택 추적만 표준 오류 스트림으로 출력되므로 모든 컨텍스트가 생략됩니다. 또한 AEM과 같은 다중 스레드 애플리케이션에서 병렬 스택 추적 출력으로 인해 문제가 발생합니다. 예외는 로깅 프레임워크에서만 기록되어야 합니다.
...
AEM에서의 로깅은 항상 로깅 프레임워크인 SLF4J를 통해 수행되어야 합니다. 표준 출력 또는 표준 오류 스트림으로 직접 출력하면 로깅 프레임워크에서 제공하는 구조 및 컨텍스트 정보가 손실되고 때때로 성능 문제가 발생합니다.
...
`/libs` 및 `/apps`(으)로 시작하는 경로는 하드코딩되지 않습니다. 이러한 경로는 일반적으로 `Sling` 검색 경로를 기준으로 저장되며 기본값은 `/libs,/apps`입니다. 절대 경로를 사용하면 프로젝트 수명 주기의 후반에만 나타나는 미묘한 결함이 발생합니다.
...
보장된 실행이 필요한 작업에는 Sling 스케줄러를 사용하지 마십시오. Sling 예약된 작업은 실행을 보장하며 클러스터된 환경과 클러스터되지 않은 환경 모두에 더 적합합니다.
...
### 더 이상 사용되지 않는 AEM API 사용 안 함 {#sonarqube-aem-deprecated}
...
일반적으로 이러한 API는 표준 Java™ *@Deprecated* 주석을 사용하여 더 이상 사용되지 않으며, 이는 `squid:CallToDeprecatedMethod`(으)로 식별됩니다.

그러나 AEM의 컨텍스트에서는 API가 더 이상 사용되지 않지만 다른 컨텍스트에서는 더 이상 사용되지 않는 경우가 있습니다. 이 규칙은 이 두 번째 클래스를 식별합니다.
...
>[!NOTE]
>
>OakPAL은 독립 실행형 Oak 저장소를 사용하여 콘텐츠 패키지의 유효성을 검사하는 프레임워크입니다. AEM 파트너이자 2019년 &#39;AEM 록스타 북미&#39; 상 수상자가 개발한 것이다.

### 로 주석이 달린 제품 API를 구현하거나 확장하지 @ProviderType {#product-apis-annotated-with-providertype-should-not-be-implemented-or-extended-by-customers}
...
### 고객 패키지의 `/libs`에 노드를 만들거나 편집하지 마십시오. {#oakpal-customer-package}
...
AEM 콘텐츠 저장소의 `/libs` 콘텐츠 트리는 고객이 읽기 전용으로 간주해야 하는 것이 오랜 모범 사례였습니다. `/libs`에서 노드 및 속성을 수정하면 주 업데이트 및 부 업데이트에 상당한 위험이 발생합니다. Adobe은 공식 채널을 통해서만 `/libs`을(를) 편집합니다.
...
[패키지에 중복된 OSGi 구성 규칙을 포함해서는 안 됨](#oakpal-package-osgi)과 마찬가지로, 이 문제는 여러 개별 콘텐츠 패키지가 동일한 노드 경로에 쓰는 복잡한 프로젝트에서 일반적인 문제입니다. 콘텐츠 패키지 종속성을 사용하면 일관된 결과를 보장할 수 있지만 중복을 완전히 피하는 것이 좋습니다.

### 기본 작성 모드는 클래식 UI가 아닙니다. {#oakpal-default-authoring}
...
### 대화 상자가 있는 구성 요소에는 Touch UI 대화 상자가 필요합니다. {#oakpal-components-dialogs}
...
### 프록시가 활성화된 클라이언트 라이브러리에 포함된 리소스는 resources라는 폴더에 있습니다 {#oakpal-resources-proxy}
...
### 특정 표준 색인 정의는 사용자 정의할 수 없습니다 {#oakpal-customizing-ootb-index}
...
AEM Cloud Service에서는 다음 표준 색인을 승인되지 않은 방식으로 수정할 수 없습니다.
...
* [Dispatcher 구성에 필수 속성이 누락되었습니다.](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---dispatcher-configuration-missing-mandatory-property)
...
* [Httpd 구성에 파일을 찾을 수 없음](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---httpd-configuration-include-file-not-found)
...
* [Dispatcher 게시 팜 캐시에 `serveStaleOnError`이(가) 활성화되어 있습니다.](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-servestaleonerror-enabled)
...
* [Dispatcher 게시 팜 캐시 `statfileslevel` 속성은 2보다 큽니다.](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-statfileslevel-property-should-be--2)

* [Dispatcher 게시 팜 `gracePeriod` 속성은 2보다 큽니다.](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-graceperiod-property-should-be--2)

* [각 Dispatcher 팜에는 고유한 이름이 있습니다](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---each-dispatcher-farm-should-have-a-unique-name)

* [Dispatcher 팜 캐시에는 `ignoreUrlParams` 규칙이 허용 목록에 추가하다 방식으로 구성되어 있습니다.](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-its-ignoreurlparams-rules-configured-in-an-allow-list-manner)

* [Dispatcher 팜 필터는 허용된 Sling 선택기를 허용 목록에 추가하다 방식으로 지정합니다](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-specify-the-allowed-sling-selectors-in-an-allow-list-manner)

* [Dispatcher 게시 팜 필터는 허용되는 Sling 접미사 패턴을 허용 목록에 추가하다 방식으로 지정합니다](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-specify-the-allowed-sling-suffix-patterns-in-an-allow-list-manner)
