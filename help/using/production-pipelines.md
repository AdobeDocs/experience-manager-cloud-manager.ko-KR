...
1. **+추가**&#x200B;를 클릭한 다음 **프로덕션 파이프라인 추가**&#x200B;를 선택합니다.
...
1. **프로덕션 파이프라인 추가** 대화 상자가 열리고 **구성** 탭이 나타나며, 이곳에서 파이프라인에 대한 여러 옵션을 정의해야 합니다. 이러한 옵션은 접을 수 있는 섹션으로 그룹화되며 다음 단계에서 설명합니다.
   ...
   1. **Source 코드** 섹션에서 파이프라인이 처리하는 코드를 검색하는 위치를 정의합니다.
      ...
      * **코드 위치** - 파이프라인이 코드를 검색해야 하는 선택한 저장소 분기의 경로를 정의합니다.
   ...
   1. **환경** 섹션에서 배포를 트리거하는 항목과 배포 환경별로 배포하는 방법을 정의합니다.
      ...
      1. **스테이지** 섹션에서 파이프라인이 스테이징 환경으로 롤아웃되는 방법을 정의할 수 있습니다.
         ...
         * **배포 트리거** - 다음과 같은 옵션을 사용하여 배포 트리거를 정의하여 파이프라인을 시작할 수 있습니다.
            ...
            * **수동** - Cloud Manager UI를 사용하여 파이프라인을 수동으로 시작합니다.
         ...
         * **중요한 지표 장애 비헤이비어** - 파이프라인 설정 또는 편집 중에 배포 관리자는 품질 게이트에서 중요한 장애가 발생했을 때 파이프라인의 비헤이비어를 정의할 수 있는 옵션을 제공합니다. 사용 가능한 옵션은 다음과 같습니다.
            ...
            * **즉시 실패** - 중요한 오류가 발생할 때마다 파이프라인이 취소됩니다. 각 실패를 수동으로 거부하는 사용자를 에뮬레이션하고 있습니다.
            * **즉시 계속** - 중요한 오류가 발생할 때마다 파이프라인이 자동으로 진행됩니다. 각 실패를 수동으로 승인하는 사용자를 에뮬레이션하고 있습니다.
         ...
         * **Dispatcher 구성** - **배포 관리자** 역할은 파이프라인이 실행될 때 AEM Dispatcher 캐시에서 무효화되거나 플러시된 콘텐츠 경로 집합을 구성할 수 있습니다. 이러한 캐시 작업은 콘텐츠 패키지가 배포된 직후에 배포 파이프라인 단계의 일부로 수행됩니다. 이러한 설정은 표준 AEM Dispatcher 비헤이비어를 사용합니다. 구성하려면 다음 작업을 수행하십시오.
      ...
      1. **프로덕션** 섹션에서 파이프라인이 프로덕션 환경으로 롤아웃되는 방법을 정의할 수 있습니다.
         ...
         * **배포 옵션** - 프로덕션 배포를 제어하는 매개변수를 정의할 수 있습니다.
            ...
            * **실행 승인 사용** - [!UICONTROL Cloud Manager] UI를 통해 **비즈니스 소유자**, **프로젝트 관리자** 또는 **배포 관리자** 역할을 가진 사용자는 배포를 수동으로 승인해야 합니다.
            * **예약됨** - 프로덕션 배포 전에 파이프라인을 중지하여 예약되도록 합니다. 이 옵션을 선택하면 스테이징 환경으로 배포한 후 파이프라인이 중지되고 사용자에게 수행할 작업을 묻는 메시지가 표시됩니다.
               * **`Now`** - 프로덕션에 즉시 배포하여 파이프라인을 효과적으로 완료합니다.
               * **날짜** - 배포를 완료해야 하는 시간을 사용자가 예약할 수 있도록 합니다.
            ...
            * **CSE 감독 사용** - 이 옵션을 선택하면 CSE(고객 성공 엔지니어)가 실제 배포를 시작하는 데 사용됩니다. 이 옵션을 활성화할 때 파이프라인을 생성하거나 편집할 때 **Deployment Manager** 역할에는 다음과 같은 옵션이 있습니다.
               ...
               * **모든 CSE** - 사용 가능한 모든 CSE가 배포를 시작할 수 있도록 허용합니다.
               * **내 CSE** - 고객에게 할당된 특정 CSE만 배포를 시작할 수 있도록 허용합니다. 이 옵션은 할당된 CSE를 사용할 수 없는 경우 CSE의 지정된 백업에도 적용됩니다.
...
1. 라이선스가 부여된 제품에 따라 AEM Sites 및 AEM Assets 성능 테스트를 구성할 수 있는 **단계 테스트** 탭으로 이동하려면 **계속**&#x200B;을 클릭하십시오.

   >[!TIP]
   >
   >**단계 테스트** 탭에서 사용할 수 있는 옵션에 대한 자세한 내용은 [코드 품질 테스트](/help/using/code-quality-testing.md#performance-testing)를 참조하십시오.

   1. **사이트 콘텐츠 배달/분산 부하 가중치** 섹션에서 세 페이지 세트 중 페이지 요청의 가중치를 기반으로 사이트 성능 테스트를 구성합니다. 필요에 따라 페이지 세트를 활성화 또는 비활성화할 수 있습니다.
