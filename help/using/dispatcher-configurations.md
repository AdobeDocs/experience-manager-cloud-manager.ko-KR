...
## 클라우드 관리자를 사용하여 발송자 구성 파일 {#using-cloud-manager-to-deploy-dispatcher-configuration-files} 배포

Cloud Manager는 일반 AEM 컨텐츠 패키지와 함께 **Git 리포지토리**&#x200B;에 저장된다고 가정할 때 웹 서버 및 Dispatcher 구성 파일을 배포할 수 있습니다.

이 기능을 활용하려면 마벤 빌드에 ***conf*** 및 ***conf.d*** 디렉토리가 두 개 이상 포함된 zip 파일이 생성되어야 합니다. 이 zip 파일은 maven-assembly-plugin을 사용하여 만들 수 있습니다. 기본 제공 [마법사](/help/using/using-the-wizard.md)를 사용하여 Cloud Manager에서 생성된 프로젝트는 프로젝트 생성 작업의 일부로 올바른 Mahen 프로젝트 구조를 만듭니다. 신규 Managed Services 고객에게 권장하는 방법입니다.

디스패처 **인스턴스**&#x200B;에 배포하면 이러한 디렉토리의 내용이 Dispatcher 인스턴스의 이러한 디렉토리 내용을 덮어씁니다. 웹 서버 및 디스패처 구성 파일은 환경별 정보를 자주 필요로 하므로 이 기능을 올바르게 사용하려면 먼저 CSE(Customer Success Engineers)와 협력하여 `/etc/sysconfig/httpd`에서 이러한 환경 변수를 설정해야 합니다.
...
1. 하드 코딩된 환경 관련 데이터(예: 퍼블리싱 렌더러 IP)를 제거하고 변수로 대체합니다.
1. 각 대상 디스패처에 대한 키-값 쌍으로 필수 변수를 정의하고 각 인스턴스에서 `/etc/sysconfig/httpd`에 추가하도록 CSE를 요청합니다.
1. 스테이지 환경에서 업데이트된 구성을 테스트한 다음 CSE에 프로덕션에 배포하도록 요청합니다.
...
특정 파일 및 디렉토리 구조는 프로젝트의 특성에 따라 달라질 수 있지만 이 예제에서는 Apache 및 Dispatcher 구성을 포함하도록 프로젝트를 구조화하는 방법에 대한 구체적인 안내서를 제공해야 합니다.

1. `dispatcher` 하위 디렉토리를 만듭니다.

   >[!NOTE]
   여기에서 아무 이름을 사용해도 되지만 이 단계에서 만든 디렉토리 이름은 6단계에서 사용한 이름과 같아야 합니다.

1. 이 하위 디렉토리에는 Maven Assembly 플러그인을 사용하여 Dispatcher zip 파일을 빌드하는 Maven 모듈이 포함됩니다. 이 작업을 시작하려면 `dispatcher` 디렉토리에서 이 내용으로 `pom.xml` 파일을 만들어 부모 참조, artifactId 및 이름을 필요에 따라 변경합니다.
...
1. 이제 디스패처 하위 디렉토리에 `src`(위의 어셈블리 설명자에서 참조됨)라는 하위 디렉토리를 만들어 실제 Apache 및 Dispatcher 구성을 저장합니다. 이 `src` 디렉터리 내에 `conf`, `conf.d`, `conf.dispatcher.d` 및 `conf.modules.d`라는 디렉토리를 만듭니다.
1. 이제 구성 파일로 `conf`, `conf.d`, `conf.dispatcher.d` 및 `conf.modules.d` 디렉토리를 채울 수 있습니다. 예를 들어 기본 구성은 이러한 파일과 심볼 링크로 구성됩니다.
...
1. 마지막으로 프로젝트 루트의 pom.xml 파일에서 디스패처 모듈을 포함할 `<module>` 요소를 추가합니다.
   ...
   >[!NOTE]
   1단계에서 설명한 바와 같이 `<module>` 요소 **의 값이 생성된 디렉토리 이름과 일치해야 합니다.**
...
1. 마지막으로 테스트하려면 프로젝트 루트 디렉토리에서 mvn 클린 패키지를 실행합니다. 출력에서 다음과 같은 선이 표시됩니다.
   ...
   이 파일의 내용을 보려면 이 파일의 압축을 해제할 수도 있습니다.
