...
전체 스택 프로덕션 파이프라인이 실행되는 동안에는 스테이지 전용 파이프라인도 프로덕션 전용 파이프라인도 실행되지 않으며 그 반대의 경우도 마찬가지입니다. 스테이지 전용 및 전체 스택 프로덕션 파이프라인에 모두 **Git 변경 시** 트리거가 구성되어 있고 동일한 분기 및 저장소를 가리키면 스테이지 전용 파이프라인만 자동으로 시작됩니다. 프로덕션 전용 파이프라인은 시작되지 않습니다 **Git 변경 시** 이는 저장소에 직접 연결되어 있지 않기 때문입니다.
