...
1. 권한 항목 옆의 더하기(`+`) 아이콘을 클릭하여 **포함된 권한 항목** 열에 추가합니다.

   * 권한 항목 옆에 있는 `i` 아이콘을 클릭하여 자세히 알아보십시오.

1. **사용 가능한 권한** 열 맨 위에 있는 **모두 추가** 단추를 클릭하여 모든 권한을 추가합니다. 마찬가지로 이전에 선택한 모든 권한을 제거하려면 **모두 제거**&#x200B;를 클릭하십시오.

1. 새 제품 프로필에 대한 권한 항목 정의를 마치면 **저장**&#x200B;을 클릭합니다.
...
1. **사용자 추가** 단추를 클릭하고 사용자 지정 권한으로 새 제품 프로필에 사용자를 할당합니다.

Admin Console 사용 방법에 대한 자세한 내용은 [기업 사용자를 위한 제품 프로필 관리](https://helpx.adobe.com/kr/enterprise/using/manage-product-profiles.html) 문서의 **제품 프로필에 사용자 및 사용자 그룹 추가** 섹션을 참조하십시오.
...
* **저장소 정보 액세스** 사용자는 이 테넌트/조직 수준의 권한을 통해 고객 프로젝트에 액세스하고 기여하기 위한 사용자 이름, 암호 및 저장소 URL을 생성할 수 있습니다.
   ...
   * 자세한 내용은 [Source 코드 저장소](/help/requirements/source-code-repository.md)를 참조하십시오.
...
| 용어 | 설명 |
|---|---|
| 미리 정의된 권한 | **비즈니스 소유자**, **배포 관리자** 등의 미리 정의된 역할 Cloud Manager의 다양한 기능을 관리합니다. 미리 정의된 역할에 대한 자세한 내용은 [역할 기반 권한](/help/requirements/role-based-permissions.md)을 참조하세요. |
