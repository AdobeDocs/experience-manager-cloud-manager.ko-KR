...
* [파이프라인을 편집](#editing-pipelines).
* [파이프라인을 삭제](#deleting-pipelines)합니다.
* [세부 정보 보기](#view-details).

파이프라인 목록의 맨 아래에는 다음과 같은 일반 옵션이 있습니다.

* **추가** - [새 프로덕션 파이프라인을 추가](/help/using/production-pipelines.md) 또는 [새 비프로덕션 파이프라인을 추가](/help/using/non-production-pipelines.md)합니다.
...
**파이프라인** 창에는 선택한 프로그램에 대한 모든 파이프라인의 전체 목록이 표시됩니다. 이 목록은 [파이프라인 카드](#pipeline-card)에서 사용할 수 있는 것보다 더 포괄적인 정보를 제공하므로 유용합니다.
...
**세부 정보 보기**&#x200B;를 클릭하여 [파이프라인 실행에 대한 세부 정보를 검토](#view-details)합니다.

## 파이프라인 실행 {#running-pipelines}

1. [my.cloudmanager.adobe.com](https://my.cloudmanager.adobe.com/)에서 Cloud Manager에 로그인한 다음 적절한 조직과 프로그램을 선택합니다.
1. **프로그램 개요** 페이지에서 **파이프라인** 카드로 이동합니다.
1. 실행하는 파이프라인 옆에 있는 줄임표 버튼을 클릭한 다음 메뉴에서 **실행**&#x200B;을 선택합니다.

   상태 열은 파이프라인 실행이 시작되는 시기를 나타냅니다.
...
1. **프로덕션 파이프라인 편집** 또는 **비프로덕션 파이프라인 편집** 대화 상자가 나타납니다. 파이프라인 생성 중에 입력한 것과 동일한 세부 정보를 편집할 수 있습니다.

   파이프라인에 사용할 수 있는 필드 및 구성 옵션에 대한 자세한 내용은 [프로덕션 파이프라인 구성](/help/using/production-pipelines.md) 및 [비프로덕션 파이프라인 구성](/help/using/non-production-pipelines.md)을 참조하십시오.
...
1. **프로그램 개요** 페이지에서 **파이프라인** 카드로 이동한 다음 실행하는 파이프라인 옆에 있는 줄임표 버튼을 클릭한 다음 메뉴에서 **삭제**&#x200B;를 선택합니다.
...
1. **프로그램 개요** 페이지에서 **파이프라인** 카드로 이동한 다음 실행하는 파이프라인 옆에 있는 줄임표 버튼을 클릭한 다음 메뉴에서 **세부 정보 보기**&#x200B;를 선택합니다.
...
**기간** 섹션을 표시하려면 **세부 정보 보기** 링크를 클릭합니다. 이 섹션에는 해당 프로그램에 대한 기록 트렌드를 기반으로 하는 파이프라인의 평균 지속 시간이 포함되어 있습니다.
