...
알림은 프로덕션 배포 중에 프로덕션 파이프라인이 시작되고 완료될 때(성공 또는 실패) 전송됩니다. 또한 **Go Live 승인** 및 **예약됨** 단계에 도달하면 알림을 보냅니다. 이러한 알림은 [!UICONTROL Experience Cloud] 알림 시스템을 통해 전송됩니다.

>[!NOTE]
>
>승인 및 예약됨 알림은 **비즈니스 소유자**, **프로그램 관리자** 및 **배포 관리자** 역할의 사용자에게만 전송됩니다.

알림은 [!UICONTROL Cloud Manager] 내 사이드바와 Adobe [!UICONTROL Experience Cloud] 전체에 표시됩니다.

새 알림이 있을 때 헤더의 벨 아이콘에 표시기가 표시됩니다.
...
**공지** 탭에는 Adobe 제품 공지가 포함됩니다. 공지는 제품에 대한 정보를 제공합니다.
...
받은 편지함의 모든 공지를 보려면 패널 하단의 **모두 보기** 옵션을 클릭하십시오.

읽지 않은 모든 알림을 읽은 것으로 표시하고 벨 아이콘에서 배지를 제거하려면 패널 하단의 **모두 읽은 상태로 표시** 옵션을 클릭합니다.
...
구독은 알림을 받는 제품과 받는 알림을 정의합니다.

![알림 구독](/help/assets/notifications-subscriptions.png)

기본적으로 모든 제품에 대한 모든 알림을 받게 됩니다. 제품에 대해 받는 알림 유형을 정의하려면 그 옆에 있는 **사용자 지정**&#x200B;을 클릭합니다.

![알림 구독 사용자 정의](/help/assets/notifications-subscriptions-customize.png)

### 우선 순위 {#priority}

우선순위 알림에는 **높음** 태그가 표시됩니다. 알림으로만 전송되도록 구성할 수 있습니다. **우선 순위** 섹션에서 우선 순위 알림에 적합한 범주를 정의할 수 있습니다.

![알림 우선 순위](/help/assets/notifications-priority.png)

드롭다운을 사용하여 우선 순위로 적합한 범주 목록에 추가합니다. 범주 이름 옆에 있는 삭제 아이콘을 클릭하여 삭제합니다.

### 경고 {#alerts}

경고는 창의 오른쪽 상단에 몇 초 동안 표시됩니다. **경고** 섹션을 사용하여 경고를 수신할 알림을 정의합니다.
...
* **경고를 해제할 때까지 화면에 유지** - 경고를 적극적으로 해제하지 않는 한 경고를 지속할지 여부를 제어합니다.
* **기간** - 경고를 화면에 유지하도록 선택하지 않은 경우 경고가 화면에 남아 있는 기간을 정의합니다.

### 이메일 {#emails}

알림은 Adobe [!UICONTROL Experience Cloud] 솔루션의 웹 사용자 인터페이스에서 사용할 수 있습니다. 전자 메일을 통해 이러한 알림을 받으려면 **전자 메일** 섹션 내에서 옵트인하십시오.
