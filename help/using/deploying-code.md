...
1. 전체 빌드 프로세스는 코드를 배포합니다.
   ...
   * 빌드 및 단위 테스트:이 단계에서는 포함된 빌드 프로세스가 실행됩니다. 빌드 환경에 대한 자세한 내용은 [빌드 환경 이해](/help/using/build-environment-details.md)를 참조하십시오.
   ...
   * 보안 테스트:이 단계에서는 AEM 환경에 애플리케이션 코드가 미치는 보안 영향을 평가합니다. 테스트 프로세스에 대한 자세한 내용은 [테스트 결과 이해](understand-your-test-results.md)를 참조하십시오.
