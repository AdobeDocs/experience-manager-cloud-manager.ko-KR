...
1. 전체 빌드 프로세스는 코드를 배포합니다.
   ...
   * 빌드 및 단위 테스트: 이 단계에서는 포함된 빌드 프로세스가 실행됩니다. 빌드 [환경에 대한 자세한 내용은 AEM 애플리케이션 프로젝트](create-an-application-project.md) 만들기를 참조하십시오.
   ...
   * 보안 테스트: 이 단계는 AEM 환경에서 응용 프로그램 코드의 보안 영향을 평가합니다. 테스트 프로세스에 대한 자세한 내용은 [테스트](understand-your-test-results.md) 결과 이해를 참조하십시오.
...
다음 섹션에서는 단계 및 제작 단계에서 AEM 및 발송자 패키지가 배포되는 방식을 설명합니다.
...
1. Cloud Manager는 각 아티팩트가 AEM 또는 발송자 패키지인지 여부를 결정합니다.
...
1. 각 AEM 아티팩트는 패키지 관리자 API를 통해 각 AEM 인스턴스에 배포되고, 패키지 종속 관계가 배포 순서를 결정합니다.

   패키지를 사용하여 새 기능을 설치하고, 인스턴스 간에 컨텐츠를 전송하고, 저장소 컨텐츠를 백업하는 방법에 대한 자세한 내용은 패키지를 사용한 작업 방법을 참조하십시오.

   >[!NOTE]
   >
   >모든 AEM 아티팩트는 작성자와 발행자 모두에 배포됩니다. 런타임 모드는 노드별 구성이 필요할 때 활용되어야 합니다. 런타임 모드를 통해 특정 용도로 AEM 인스턴스를 조정할 수 있는 방법에 대한 자세한 내용은 실행 모드를 참조하십시오.
...
1. AEM 패키지를 publish1에 배포하고 발송자 패키지를 dispatcher1에 플러시합니다.
1. dispatcher1을 로드 밸런서에 다시 넣습니다.
1. dispatcher1이 다시 서비스를 받으면 로드 밸런서에서 dispatcher2를 분리합니다.
1. AEM 패키지를 게시2에 배포하고 발송자 패키지를 dispatcher2에 플러시합니다.
