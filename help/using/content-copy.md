...
콘텐츠 세트가 복사할 콘텐츠를 정의합니다. 콘텐츠 세트에는 복사할 변경 가능한 콘텐츠가 있는 JCR 경로 목록이 포함됩니다. 콘텐츠는 소스 환경에서 대상 환경으로 이동합니다. 이 작업은 동일한 Cloud Manager 프로그램 내에서 수행됩니다.
...
콘텐츠를 복사하려면 먼저 콘텐츠 세트를 정의해야 합니다. 정의되면 콘텐츠 세트를 재사용하여 콘텐츠를 복사할 수 있습니다.
...
1. (선택 사항) 필요한 경우 앞의 두 단계를 반복하여 최대 50개의 경로를 추가합니다. 그렇지 않으면 다음 단계를 계속 진행합니다.

   ![콘텐츠 세트에 경로 추가](/help/assets/add-content-set-paths.png)

1. (선택 사항) 콘텐츠 세트를 세분화하기 위해 제외해야 하는 포함된 콘텐츠 경로 내에 하위 경로를 선택적으로 지정할 수 있습니다.

   1. 제외하려는 포함된 콘텐츠 경로의 오른쪽에서 ![폴더 삭제 아이콘](https://spectrum.adobe.com/static/icons/workflow_18/Smock_FolderDelete_18_N.svg)을 클릭합니다.
   ...
   1. 제외된 경로를 더 추가하려면 단계 i, ii 및 iii를 반복하십시오. 제한은 없습니다. 그렇지 않으면 다음 단계를 계속 진행합니다.
...
제외된 하위 경로를 표시하려면 구성된 경로를 확장합니다.
...
1. (선택 사항) 다음 중 하나를 수행합니다.
   ...
   1. 대상 환경에서 제외된 경로를 *유지*&#x200B;하려면 **`Do not delete excluded paths from destination`**&#x200B;을 선택합니다. 이 설정으로 콘텐츠 세트에 지정된 제외 경로가 그대로 유지됩니다.
   1. 대상 환경에서 제외된 경로를 *제거*&#x200B;하려면 **`Do not delete excluded paths from destination`**&#x200B;의 선택을 취소합니다. 이 설정으로 콘텐츠 세트에 지정된 제외된 경로가 삭제됩니다.
...
* 콘텐츠 복사는 동일한 계층 내에서만 수행할 수 있습니다. 즉, 작성자-작성자 또는 게시-게시를 의미합니다.
...
* 콘텐츠 복사는 상위 환경에서 선택한 하위 환경으로 Dynamic Media 메타데이터와 자산을 복제합니다. 복사된 에셋은 하위 환경의 [DAM 에셋 처리 워크플로우](https://experienceleague.adobe.com/ko/docs/experience-manager-65/content/assets/using/assets-workflow)를 사용하여 다시 처리해야 합니다. 각 Dynamic Media 구성을 사용하려면 이 재처리가 필요합니다.
* [자산 크기가 2GB보다 큰 Dynamic Media 구성이 활성화됨](https://experienceleague.adobe.com/ko/docs/experience-manager-65/content/assets/dynamic/config-dms7#optional-config-dms7-assets-larger-than-2gb)은(는) 지원되지 않습니다.
