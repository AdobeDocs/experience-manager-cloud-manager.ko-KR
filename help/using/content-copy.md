---
title: 콘텐츠 복사 도구
description: Cloud Manager 콘텐츠 복사 도구를 사용하면 사용자가 테스트를 위해 AMS 호스팅된 AEM 6.x 프로덕션 환경에서 하위 환경으로 변경 가능한 주문형 콘텐츠를 복사할 수 있습니다.
...
Cloud Manager 콘텐츠 복사 도구를 사용하면 사용자가 테스트를 위해 AMS 호스팅된 AEM 6.x 프로덕션 환경에서 하위 환경으로 변경 가능한 주문형 콘텐츠를 복사할 수 있습니다.
...
1. 화면의 오른쪽 상단 근처에서 **콘텐츠 세트 추가** 버튼을 클릭합니다.
...
1. 콘텐츠 세트를 구체화하거나 제한해야 하는 경우 하위 경로를 제외할 수 있습니다.
   ...
   1. **경로 제외**&#x200B;를 클릭합니다.
   1. 필요에 따라 제외할 경로를 추가하려면 **하위 경로 제외 추가**&#x200B;를 다시 클릭합니다.
...
1. 필요에 따라 지정한 경로를 변경할 수 있습니다.
   ...
   1. **편집** 및 **삭제** 옵션을 표시하려면 경로 옆에 있는 줄임표 버튼을 클릭합니다.
...
1. **콘텐츠 복사** 대화 상자에서 콘텐츠 복사 액션의 소스 및 대상 환경을 지정합니다.
   * 대상 환경의 지역은 소스 환경 지역과 동일하거나 하위 집합이어야 합니다.

1. 대상 환경에서 제외 경로를 삭제하거나 유지할 수 있습니다. 콘텐츠 세트에 지정된 `exclude paths`를 유지하려면 `Do not delete exclude paths from destination` 확인란을 선택합니다. 확인란을 선택되지 않은 상태이면 대상 환경에서 제외 경로가 삭제됩니다.
