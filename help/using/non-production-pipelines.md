...
|   | **전략 작성** | <ul><li>**전체 빌드** - 매번 저장소의 모든 모듈을 빌드합니다.<li>BETA **스마트 빌드** - 마지막 커밋 이후 변경된 모듈만 빌드합니다.<br>비프로덕션 파이프라인에서 [스마트 빌드를 사용하는 방법에 대해 자세히 알아보세요](#about-smart-build).</li></ol>**중요**: 스마트 빌드는 코드 품질 파이프라인 및 개발 전체 스택 코드 배포 파이프라인에만 사용할 수 있습니다. |
