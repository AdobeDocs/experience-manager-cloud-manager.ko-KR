...
Cloud Manager를 사용하여 코드를 배포할 비프로덕션 파이프라인을 만들고 구성하는 방법에 대해 알아보십시오. Cloud Manager에서 파이프라인이 작동하는 방식에 대한 보다 개념적인 개요는 [CI/CD 파이프라인](/help/overview/ci-cd-pipelines.md)을 참조하십시오.

## 개요 {#overview}

[!UICONTROL Cloud Manager]의 **파이프라인** 타일을 사용하여 **배포 관리자**&#x200B;는 두 가지 유형의 파이프라인을 만들 수 있습니다.

* **프로덕션 파이프라인** - 프로덕션 파이프라인은 프로덕션에 소스 코드를 배포하기 위한 일련의 조정된 단계로 구성된 특별히 빌드된 파이프라인입니다.
...
* **코드 품질 파이프라인** - Git 분기의 코드에 대해 코드 품질 검사를 실행하고 빌드 및 코드 품질 단계를 실행합니다.
...
|   | **전략 작성** | <ul><li>**전체 빌드** - 매번 저장소의 모든 모듈을 빌드합니다.<li>**스마트 빌드** - 마지막 커밋 이후에 변경된 모듈만 빌드합니다.<br>비프로덕션 파이프라인에서 [스마트 빌드를 사용하는 방법에 대해 자세히 알아보세요](#about-smart-build).</li></ol> |
|   | **웹 계층 구성 무시** 확인란 | 전체 스택 코드 파이프라인에서 웹 계층 구성의 배포를 건너뛰려면 이 옵션을 선택합니다. 파이프라인 코드와 함께 웹 계층 구성을 배포하려면 옵션을 선택하지 않은 상태로 둡니다. |
| **파이프라인** | **경험 감사** 확인란 | 파이프라인에 경험 감사 단계를 포함하려면 이 옵션을 선택합니다. 활성화되면 Source 코드 탭 뒤에 파이프라인에 경험 감사 단계가 포함됩니다. |
...
## 비프로덕션 파이프라인에서 스마트 빌드 사용{#about-smart-build}
...
* 코드 품질 파이프라인.
* 개발, 스테이징 및 비프로덕션 전체 스택 배포 파이프라인.
...
독립 모듈이 많은 프로젝트는 가장 큰 개선 효과를 볼 수 있습니다.
...
* 종속성 그래프 외부의 변경 사항은 재빌드를 트리거하지 않습니다.
* 일부 플러그인은 캐싱과 완전히 호환되지 않습니다.
