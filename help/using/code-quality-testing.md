...
[이 링크](/help/assets/CodeQuality-rules-latest-AMS.xlsx)를 사용하여 규칙의 최신 전체 목록을 다운로드할 수 있습니다.
...
코드 품질 테스트 결과는 다음 표에 요약된 등급으로 제공됩니다.
...
>[!NOTE]
>
>자세한 내용은 [SonarQube의 지표 정의](https://docs.sonarsource.com/sonarqube-server/user-guide/code-metrics/metrics-definition)를 참조하십시오.
...
이러한 경우 소스 코드는 규칙 ID를 주석 속성으로 지정하는 표준 Java `@SuppressWarnings` 주석으로 추가할 수 있습니다. 예를 들어 한 가지 일반적인 긍정 오류는 하드코딩된 암호를 탐지하는 SonarQube 규칙이 하드코딩된 암호를 탐지하는 방법에 대해 지나치게 민감할 수 있다는 것입니다.
...
Cloud Manager는 AEM Sites 프로그램에 대한 성능 테스트를 실행합니다. 성능 테스트는 실제 사용자를 시뮬레이션하는 가상 사용자(컨테이너)를 만들어 트래픽을 시뮬레이션하는 스테이징 환경의 페이지에 액세스함으로써 30분 동안 실행됩니다. 이러한 페이지는 크롤러를 사용하여 발견됩니다.

#### 가상 사용자 {#virtual-users}

Cloud Manager은 **비즈니스 소유자** 역할에 의해 설정된 KPI(응답 시간 및 페이지 보기 수/분)를 기반으로 가상 사용자 또는 컨테이너를 만듭니다. 이러한 KPI는 [프로그램을 생성하거나 편집](/help/getting-started/program-setup.md)할 때 설정됩니다.

정의된 KPI를 기준으로 실제 사용자를 시뮬레이션하는 컨테이너는 최대 10개까지 만들어집니다. 테스트를 위해 선택된 페이지는 분할되어 각 가상 사용자에게 할당됩니다.
...
* **기타 라이브 페이지** - 방문 빈도가 높은 라이브 페이지 상위 25개 밖에 있어 방문 빈도가 높지는 않지만 테스트에 중요한 페이지를 테스트합니다. 방문 빈도가 높은 라이브 페이지와 유사하게, 이 페이지는 액세스 로그에서 추출되며 스테이징 환경에도 존재해야 합니다.
...
API 사용 방법에 대한 자세한 내용은 [사용자 파이프라인 변수 패치](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api#operation/patchPipelineVariables) API 설명서를 참조하십시오.
...
고객은 테스트를 위해 자신의 자산을 업로드할 수 있습니다. 이 프로세스는 **파이프라인 설정** 또는 **편집** 화면에서 수행할 수 있습니다. JPEG, PNG, GIF, BMP 및 Adobe 형식과 같은 일반적인 이미지 형식이 지원됩니다.
...
>[!TIP]
>
>자세한 내용은 [프로덕션 파이프라인 구성](/help/using/production-pipelines.md)을 참조하세요. 프로그램을 설정하고 KPI를 정의하는 방법은 [프로그램 설정](/help/getting-started/program-setup.md)을 참조하세요.
...
핵심 최적화는 프로젝트가 단일 &#39;모든&#39; 패키지를 출력하는 것입니다. 이 패키지에는 빌드에서 생성된 다른 콘텐츠 패키지가 포함되어 있으며, 건너뛰기로 표시됩니다. Cloud Manager가 이 시나리오를 감지하면 “모든” 패키지의 압축을 푸는 대신 개별 콘텐츠 패키지가 직접 스캔되고 종속성에 따라 정렬됩니다. 예를 들어 다음 빌드 출력을 고려해 보십시오.

* `all/myco-all-1.0.0-SNAPSHOT.zip` (content-package)
* `ui.apps/myco-ui.apps-1.0.0-SNAPSHOT.zip` (skipped-content-package)
* `ui.content/myco-ui.content-1.0.0-SNAPSHOT.zip` (skipped-content-package)

`myco-all-1.0.0-SNAPSHOT.zip` 내의 항목만 건너뛴 두 개의 콘텐츠 패키지인 경우 &quot;모든&quot; 콘텐츠 패키지 대신 임베드된 두 개의 패키지가 스캔됩니다.
