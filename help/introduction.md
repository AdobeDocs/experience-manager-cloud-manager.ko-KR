...
AMS용 Cloud Manager(Adobe Managed Services)에 대해 알아보고 조직이 클라우드에서 Adobe Experience Manager을 자체 관리하는 방법을 알아보려면 여기에서 시작하십시오.
...
[!UICONTROL Cloud Manager]의 주요 기능 중 하나는 최적화된 CI/CD 파이프라인을 사용하여 사용자 지정 코드 또는 업데이트(예: 웹 사이트에 새 구성 요소 추가)의 게재 속도를 단축하는 기능입니다.

[!UICONTROL Cloud Manager] UI를 통해 CI/CD 파이프라인을 구성하고 시작할 수 있습니다. 이 파이프라인의 일부로 고품질 애플리케이션만 프로덕션 환경에 전달되도록 철저한 코드 검사가 실행됩니다.

[!UICONTROL Cloud Manager]의 UI에서 파이프라인을 구성하는 방법에 대한 자세한 내용은 [프로덕션 파이프라인 구성](/help/using/production-pipelines.md) 및 [비프로덕션 파이프라인 구성](/help/using/non-production-pipelines.md)을 참조하십시오.
...
배포 트리거와 관계없이, 배포가 트리거될 때마다 품질 검사가 CI/CD 파이프라인 실행의 일부로 항상 수행됩니다. 품질 검사에는 코드 검사, 보안 테스트 및 성능 테스트가 포함되며, 이 모든 기능은 사용자나 파트너의 노력 없이 표준 기능으로 제공됩니다.

코드 배포 및 품질 검사에 대한 자세한 내용은 [코드 배포](/help/using/code-deployment.md)를 참조하십시오.

## Cloud Manager의 옵션 기능 {#optional-features-in-cloud-manager}

Cloud Manager은 특정 환경 설정 및 필요에 따라 프로젝트에 도움이 되는 추가 고급 기능을 제공합니다. 자세한 내용은 고객 성공 엔지니어(CSE) 또는 Adobe 담당자에게 문의하십시오.

### 자동 크기 조정 {#autoscaling}

프로덕션 환경에 비정상적으로 높은 부하가 걸리면 [!UICONTROL Cloud Manager]에서 추가 용량의 필요성을 감지하고 자동 크기 조정 기능을 사용하여 자동으로 추가 용량을 프로비저닝합니다.

이러한 이벤트에서 [!UICONTROL Cloud Manager]은(는) 자동으로 자동 크기 조정 제공 프로세스를 트리거하고, 자동 크기 조정 이벤트 알림을 보내고, 몇 분 안에 추가 용량을 프로비저닝합니다. 추가 용량은 실행 중인 Dispatcher/게시 노드의 시스템 사양과 일치하도록 프로덕션 환경 및 동일한 지역에서 프로비저닝됩니다.

자동 확장 기능은 Dispatcher/게시 계층에 적용되며, 가로 크기 조정을 사용하여 Dispatcher/게시 쌍의 세그먼트를 1~10개 추가합니다. 제공된 추가 용량은 Adobe CSE(고객 성공 엔지니어)가 결정하는 10일 이내 기간에 수동으로 조정됩니다.
...
* 설정의 일부로 녹색 로드 밸런서가 생성됩니다. 이 로드 밸런서는 변경되지 않으며 녹색 또는 &quot;테스트&quot; URL의 대상입니다.
* 파란색/녹색 배포 중에 기존 Dispatcher/게시 계층의 정확한 복제본이 생성됩니다.
...
* 파란색/녹색은 Dispatcher/게시 쌍에만 사용할 수 있습니다.
* 미리보기 Dispatcher/게시 쌍은 파란색/녹색 배포의 일부가 아닙니다.
* 모든 Dispatcher/게시 쌍은 다른 Dispatcher/게시자 쌍과 동일합니다.
