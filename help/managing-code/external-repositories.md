...
1. 각 필드에 저장소에 대한 다음 세부 정보를 입력합니다.
   ...
   | **저장소 URL** | 필수. 저장소의 URL.<br><br>GitHub 호스팅 리포지토리를 사용하는 경우 경로는 `.git`로 끝나야 합니다.<br>예: *`https://github.com/org-name/repo-name.git`*(URL 경로는 일러스트레이션용임).<br><br>외부 리포지토리를 사용하는 경우 <br>`https://git-vendor-name.com/org-name/repo-name.git`<br> 또는<br>`https://self-hosted-domain/org-name/repo-name.git`<br>다음 URL 경로 형식을 사용하고 Git 공급업체와 일치해야 합니다. |
...
1. **저장**&#x200B;을 선택하여 저장소를 추가합니다.
   ...
   외부 저장소의 소유권을 확인하려면 액세스 토큰을 제공하십시오.

1. 액세스할 수 있도록 외부 저장소의 소유권을 확인하려면 **개인 저장소 소유권 확인** 대화 상자에서 액세스 토큰을 제공한 다음 **확인**&#x200B;을 클릭하십시오.
...
1. Cloud Manager에 필요한 이벤트를 전송하려면 Webhook을 구성합니다. 다음 표를 사용하여 Git 공급자에 대한 올바른 이벤트를 결정하십시오.
...
| 필수 웹후크 이벤트 및 인증 |
| --- |
| 이러한 이벤트를 통해 Cloud Manager은 가져오기 요청의 유효성을 검사하고, 코드 푸시에 응답하고, 파이프라인 조정을 위한 댓글과 상호 작용할 수 있습니다.<br>다음 필수 웹후크 이벤트를 트리거하도록 웹후크가 설정되어 있는지 확인하십시오<ul><li>코드 푸시됨</li><li>끌어오기 요청에 댓글 남김</li><li>끌어오기 요청 생성됨</li><li>끌어오기 요청 업데이트됨</li></ul>인증 설정:<br>1. **기본 인증 사용자 이름** 필드에 `cloudmanager`.<br>2을(를) 입력합니다. **기본 인증 암호** 필드에 Cloud Manager 사용자 인터페이스에서 생성한 Webhook 암호를 입력합니다. |
...
확인 표시가 생성되면 다음 스크린샷과 같이 표시됩니다. `GitHub.com`과(와) 중요한 차이점은 `GitHub.com`이(가) 확인 실행을 사용하는 반면 GitHub Enterprise(개인 액세스 토큰 사용)는 커밋 상태를 생성한다는 것입니다.
...
![코드 품질 유효성 검사가 완료되면](/help/managing-code/assets/repository-webhook-gitlab2.png)
