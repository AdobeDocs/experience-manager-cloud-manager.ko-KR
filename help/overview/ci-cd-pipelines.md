...
* 환경에 해당하는 웹 계층 구성 파이프라인이 존재하지 않는 경우 사용자는 전체 스택 파이프라인을 구성할 때 Dispatcher 구성을 포함하거나 무시할 수 있습니다.
...
웹 계층 구성 파이프라인을 사용하면 HTTPD/Dispatcher 구성을 AEM 런타임에 독점적으로 배포하여 다른 코드 변경과 분리할 수 있습니다. Dispatcher 구성 변경만 배포하려는 사용자에게 이를 신속하게 할 수 있는 효율적인 방법을 제공하는 간소화된 파이프라인입니다.
...
이제 Cloud Manager에서는 빌드 프로세스를 가속화하기 위해 모듈 수준 캐싱을 사용하는 최적화된 빌드 전략 **스마트 빌드**&#x200B;를 사용합니다. 각 빌드 동안 변경된 모듈만 다시 빌드되고 변경되지 않은 모듈은 캐시에서 재사용됩니다.

스마트 빌드는 코드 품질 및 전체 스택 배포 파이프라인(개발, 스테이지, 프로덕션)에 사용할 수 있습니다.
...
* **중요** - 게이트에 의해 식별된 중요한 문제는 파이프라인을 일시 중지 상태로의 전환을 초래합니다. 배포 관리자, 프로젝트 관리자 또는 비즈니스 리드는 문제를 재정의하여 파이프라인을 진행할 수 있습니다. 또는 파이프라인이 오류로 중단되는 경우 문제를 수락할 수 있습니다.
