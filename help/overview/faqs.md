...
Java 8에서 Java 11로 전환하려고 하면 AEM Cloud Manager 빌드가 실패합니다. 다음 오류가 발생하면 `maven-scr-plugin`을 제거하고 모든 OSGi 주석을 OSGi R6 주석으로 변환해야 합니다.
...
이 문제는 Cloud Manager이 다른 버전의 Java를 사용하여 Maven 명령을 실행하므로 발생합니다. 이 버전은 코드를 컴파일하는 데 사용되는 버전과 다릅니다. `maven-enforcer-plugin` 구성에서 `requireJavaVersion`을 생략합니다.
...
## Adobe Managed Services 환경의 성능 테스트 단계에서 Cloud Manager 배포가 실패합니다. 중요한 지표를 전달하려면 이 문제를 어떻게 해결해야 합니까? {#debug-critical-metrics}

이 질문에 대한 결정적인 대답은 단 한 개도 없다. 그러나 성능 테스트 단계에 대한 다음 사항은 유용합니다.

* 이 단계는 웹 성능 단계입니다. 즉, 웹 브라우저에서 페이지를 로드하는 데 걸리는 시간에 관한 것입니다.
* 테스트 중에 Cloud Manager 인프라 내의 Chrome 브라우저에서 결과 .csv 파일 로드 로 나열된 URL입니다.
...
버전을 변경하지 않은 경우 후속 배포를 설치할 수 있습니다. 개발자 배포에서는 Maven 빌드에 대해 자동 버전이 추가되거나 생성되지 않습니다.
...
스테이지 및 프로덕션 배포에서 사용자 정의 버전을 사용하려면 `1.0.0`과(와) 같은 적절한 3부분으로 구성된 Maven 버전을 설정합니다. 프로덕션에 배포할 때마다 버전을 늘립니다.
...
## Maven 빌드가 Cloud Manager 배포에 실패하지만 오류 없이 로컬로 빌드됩니다. 원인이 무엇입니까? {#maven-build-fail}
...
`aio` 명령을 통해 파이프라인 변수를 나열하거나 설정하려고 하면 다음과 같은 403 오류가 표시됩니다.
